# Container-first rimfrost workflows
...
- `rust/crates/rusty-claude-cli/src/main.rs` exposes that state through the `rimfrost sandbox` / `cargo run -p rusty-claude-cli -- sandbox` report.
...
```bash
docker build -t rimfrost-dev -f Containerfile .
```

### Podman

```bash
podman build -t rimfrost-dev -f Containerfile .
```
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/rimfrost-target \
  -w /workspace/rust \
  rimfrost-dev \
...
If you want to run `rimfrost` against a second checkout while keeping `rimfrost` itself mounted read-write:
...
- Running with `CARGO_TARGET_DIR=/tmp/rimfrost-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.
