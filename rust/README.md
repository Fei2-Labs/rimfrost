# 🦞 RimFrost — Rust Implementation

A high-performance Rust rewrite of the RimFrost CLI agent harness. Built for speed, safety, and native tool execution.
...
| Config file hierarchy (`.rimfrost.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
`rimfrost acp` is a local discoverability surface for editor-first users: it reports the current ACP/Zed status without starting the runtime. As of April 16, 2026, rimfrost does **not** ship an ACP/Zed daemon entrypoint yet, and `rimfrost acp serve` is only a status alias until the real protocol surface lands.
...
Notable rimfrost-first surfaces now available directly in slash form:
...
└── crates/
    ...
    ├── rusty-claude-cli/   # Main CLI binary (`rimfrost`)
