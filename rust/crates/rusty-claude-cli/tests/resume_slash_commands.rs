...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  rimfrost --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".rimfrost")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".rimfrost").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = run_claw_with_env(
        ...
        &[("RIMFROST_CONFIG_HOME", config_home.to_str().expect("utf8 path"))],
    );
